...
Users can complete the task based on their roles configured in the task configuration module.
---
...
#### 6.1.10 Workflow Integration & Post-Action Behavior

{Describe how this feature integrates with the {{defaultClientId}} workflow engine — downstream task creation, rejection/send-back flows, post-submission effects, and any cascading effects on other tasks or processes. This is the single canonical location for all workflow and post-action behavior.}
...
| PA-2 | {Post-action — e.g., Asset record update} | {Description — e.g., Asset contact is updated with selected vendor.} |
| PA-3 | {Post-action — e.g., Notification sent} | {Description — e.g., Email sent to stakeholders per §13.} |
...
{Provide a comprehensive field-level specification for all inputs in this feature. This extended table captures control types, allowed values, mandatory status, validation rules, prepopulation sources, dynamic visibility, calculated fields, read-only status, data sources, constraints, and display logic. The "Prepopulated" and "Data Source" columns replace the former standalone Prepopulating Data section — specify the upstream task or system source directly in each row.}
...
### 10.1 APIs — Outbound Integrations
...
### 13.1 Task Creation Notification
...
### 13.2 Task Submission Notifications
...
### 16.1 {Screen / View 1 — e.g., Landing Page}
...
### 16.2 {Screen / View 2 — e.g., Add / Create}
...
### 17.1 Attributes Excluded from 2.0
...
### 17.2 Stored Procedures & Technical Dependencies
