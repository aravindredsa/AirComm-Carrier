...
- Keep this prompt's template, output path, and completeness requirements as final authority when more specific.
...
Use `templates/ba/ui-analysis-template.md` as the required output structure for the final document.

Output: generate an md and docx file in `artifacts/ui-analysis/` and title it appropriately.
Filename must include today's date in `YYYY-MM-DD` format (example: `UI_Analysis_2026-04-30.md` and `UI_Analysis_2026-04-30.docx`).
...
```bash
node .github/initializer/tools/log-prompt-usage.js --prompt-path=.github/prompts/ba-ui-analysis.prompt.md
```
